...
const HEADERS = {
  ...
  // Written back by the Apps Script when it creates the event. Never typed by hand.
  calendar_event_id:['calendar event id', 'event id (calendar)', 'gcal id'],
  ...
  attendees_emails: ['attendees emails', 'attendee emails'],
  // Written by /api/interest, never by hand and never by the Apps Script. Personal addresses.
  interested_emails:['interested emails', 'interested'],
...
export function normalizeRows(values) {
  ...
  for (let i = header.index + 1; i < values.length; i++) {
    ...
    records.push({ sheetRow, idFields, event: {
      ...
      calendar_event_id: cell(row, 'calendar_event_id'),
...
const INTERNAL = new Set(['notes', 'event_owner', 'owner', 'comments',
  // Written back by the Apps Script: the guest list, with personal email addresses in it.
  'attendees', 'participants', 'guest_list', 'attendees_emails', 'interested_emails',
  // The row this came from. Server-side plumbing (/api/interest writes by it); of no use to
  // a browser, and one fewer thing said about the shape of the sheet.
  'source_row',
  // A joining URL. Publishing it would let anyone walk into the meeting.
  'meeting_link', 'video_link', 'join_link']);
