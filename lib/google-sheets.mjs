...
async function getAccessToken({ email, privateKey }) {
  // Keyed by the account, so rotating the key or changing the account cannot serve a stale one.
  const cacheKey = `${email}:${privateKey.length}:${privateKey.slice(-24)}`;
  ...
  if (!res.ok) {
    ...
    throw new SheetError('auth_failed',
      `Google refused the service account (${res.status}). ${detail.slice(0, 300)}`,
  ...
  tokenCache = { token: data.access_token, key: cacheKey,
...
export async function fetchSheetValues({ email, privateKey, spreadsheetId, range }) {
  ...
  const token = await getAccessToken({ email, privateKey: normalizePrivateKey(privateKey) });
