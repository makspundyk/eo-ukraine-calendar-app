...
export class SheetError extends Error {
  constructor(reason, message, { status, publicMessage } = {}) {
    ...
    this.publicMessage = publicMessage || 'The live calendar could not be read, so these are sample events.';
  }
}
...
function pemToPkcs8(pem) {
  ...
  if (!body) throw new SheetError('bad_private_key', 'The private key is empty.',
    { publicMessage: 'This is not set up yet. Tell the chapter and use the calendar buttons in the meantime.' });
  let binary;
  try {
    binary = atob(body);
  } catch {
    throw new SheetError('bad_private_key', 'The private key is not valid base64 — a paste that lost characters, or an extra quote.',
      { publicMessage: 'This is not set up yet. Tell the chapter and use the calendar buttons in the meantime.' });
  }
...
export async function getAccessToken({ email, privateKey, scope = SCOPES.sheets, subject }) {
  // Normalised HERE, not by the caller. It used to be applied in fetchSheetValues only, so a
  // key pasted into a dashboard — where the newlines become the two characters \ and n —
  // worked for reading the sheet and failed for everything else. The bug was invisible locally,
  // because a key read from the JSON file already has real newlines. One gate, no exceptions.
  ...
  const cacheKey = `${email}|${scope}|${subject || ''}|${pem.slice(-24)}`;
  ...
  try {
    ...
    throw new SheetError('bad_private_key',
      ...
      + 'account JSON, BEGIN/END lines included — check that the value in Cloudflare still has '
      + 'its -----BEGIN PRIVATE KEY----- and -----END PRIVATE KEY----- lines.',
      { publicMessage: 'This is not set up yet. Tell the chapter and use the calendar '
  ...
  if (!res.ok) {
    ...
    throw new SheetError('auth_failed',
      ...
      { status: res.status,
        publicMessage: 'The calendar could not be reached just now. Try again in a moment.' });
  }
...
export async function fetchSheetValues({ email, privateKey, spreadsheetId, range }) {
  ...
  const token = await getAccessToken({ email, privateKey, scope: SCOPES.sheets });
  ...
  if (res.status === 403) {
    throw new SheetError('not_shared',
      ...
      { status: 403, publicMessage: 'The live calendar is not readable, so these are sample events.' });
  }
  if (res.status === 404) {
    throw new SheetError('not_found',
      'No spreadsheet with that id, or the tab named in the range does not exist.',
      { status: 404, publicMessage: 'The live calendar could not be found, so these are sample events.' });
  }
  if (!res.ok) {
    const detail = await res.text().catch(() => '');
    throw new SheetError('sheets_failed',
      `Sheets API returned ${res.status}. ${detail.slice(0, 300)}`,
      { status: res.status, publicMessage: 'The live calendar could not be read, so these are sample events.' });
  }

  const { values } = await res.json();
  if (!Array.isArray(values) || values.length === 0) {
    throw new SheetError('empty', 'The sheet range came back with no rows at all.',
      { publicMessage: 'The live calendar has no events in it, so these are samples.' });
  }
  return values;
}
