/**
 * The subscriber list, kept in the sheet.
 * ===========================================================================
 * A second tab, `EventCalendarSubscriptions`, so the chapter owns its list in the same place
 * it owns everything else — no database, no third-party mailing tool, and it can be sorted,
 * filtered and exported like any other sheet.
 *
 *   Email | Date Subscribed | Full Name | Unsubscribed | Token
 *
 * `Unsubscribed` is a checkbox. Anyone whose box is NOT ticked gets invited; ticking it by
 * hand in the sheet works exactly as well as clicking the link in an invitation, which is what
 * a chapter secretary will actually do when somebody asks them in person.
 *
 * `Token` is the only column nobody types. It is what makes an unsubscribe link safe: a random
 * string per subscriber, so the link cannot be guessed and nobody can remove anybody else by
 * editing an address in a URL — and that link travels inside a calendar invitation, which gets
 * forwarded.
 *
 * ONE EXTRA SCOPE, USED FOR BOTH READING AND WRITING THIS TAB.
 * `https://www.googleapis.com/auth/spreadsheets`, on the same domain-wide delegation entry.
 * It grants read as well, so there is no reason to ask for `spreadsheets.readonly` too — and
 * asking for it separately fails with `unauthorized_client` unless an admin authorised both.
 *
 * The calendar tab is read by a different path entirely: no impersonation, readonly, and the
 * sheet shared directly with the service account. That path is unaffected by any of this.
 * Until this scope is authorised, subscribing answers `not_enabled` and the site keeps
 * offering the .ics feed, which needs nothing.
 */
import { getAccessToken, SCOPES, SheetError } from './google-sheets.mjs';
import { readConfig } from './calendar.mjs';

const SHEETS_API = 'https://sheets.googleapis.com/v4/spreadsheets';
export const TAB = 'EventCalendarSubscriptions';
export const COLUMNS = ['Email', 'Date Subscribed', 'Full Name', 'Unsubscribed', 'Token'];

/** Same rule as the invitation form: reject what is certainly not an address, nothing more. */
export const looksLikeEmail = (v) =>
  typeof v === 'string' && v.length <= 254
  && /^[^\s@,;]+@[^\s@,;.]+\.[^\s@,;]{2,}$/.test(v.trim());

/** 160 bits of randomness. Long enough that guessing is not a strategy. */
export function newToken() {
  const bytes = crypto.getRandomValues(new Uint8Array(20));
  return Array.from(bytes, (b) => b.toString(36).padStart(2, '0')).join('').slice(0, 28);
}

function config(env) {
  const base = readConfig(env);
  return { ...base, impersonate: env.GOOGLE_IMPERSONATE_USER };
}

async function token(cfg, scope) {
  try {
    return await getAccessToken({ email: cfg.email, privateKey: cfg.privateKey,
                                  scope, subject: cfg.impersonate });
  } catch (err) {
    // Almost always one thing: the write scope has not been authorised in the admin console.
    // Saying "the calendar service could not authenticate" here points at the wrong surface.
    throw new SheetError(err.reason || 'auth_failed',
      `${err.message} (scope: ${scope})`,
      { publicMessage: 'Signing up is not switched on yet. Take the calendar feed instead.' });
  }
}

const ready = (cfg) => Boolean(cfg.email && cfg.privateKey && cfg.spreadsheetId && cfg.impersonate);

/* --------------------------------------------------------------------- read */

/** Every row, as objects. The header is row 1 of this tab — it is ours, so it is fixed. */
export async function readSubscribers(env) {
  const cfg = config(env);
  if (!ready(cfg)) throw new SheetError('not_enabled', 'Subscriptions are not configured.',
    { publicMessage: 'Signing up is not switched on yet. Take the calendar feed instead.' });

  // The WRITE scope, even to read. `spreadsheets` already grants read, so subscriptions need
  // exactly one entry in the delegation rather than two — and asking for readonly separately
  // fails with `unauthorized_client` unless an admin happened to authorise both.
  const t = await token(cfg, SCOPES.sheetsWrite);
  const res = await fetch(
    `${SHEETS_API}/${encodeURIComponent(cfg.spreadsheetId)}/values/${encodeURIComponent(TAB)}`,
    { headers: { authorization: `Bearer ${t}` } });

  if (res.status === 400 || res.status === 404) return { rows: [], missingTab: true };
  if (!res.ok) {
    throw new SheetError('read_failed', `Could not read ${TAB} (${res.status}).`,
      { publicMessage: 'That did not work just now. Try again in a moment.' });
  }
  const { values = [] } = await res.json();
  if (!values.length) return { rows: [], missingTab: false, needsHeader: true };

  const head = values[0].map((h) => String(h || '').trim().toLowerCase());

  // Is row 1 actually a header? An empty tab that somebody created by hand has none, and
  // treating the first SUBSCRIBER as the header hides them — so every visit looks new, a
  // duplicate row is appended, and nobody can ever be found to unsubscribe.
  if (!head.includes('email')) {
    return { rows: [], missingTab: false, needsHeader: true, firstRowIsData: true };
  }
  const at = (name) => head.indexOf(name.toLowerCase());
  // A Sheets checkbox reads back as the boolean true; typed by hand it is a string.
  const ticked = (v) => v === true || ['true', 'yes', 'y', '1', '✓', 'x'].includes(
    String(v ?? '').trim().toLowerCase());

  const rows = values.slice(1).map((r, i) => ({
    rowNumber: i + 2,                                  // 1-based, header is row 1
    email: String(r[at('Email')] ?? '').trim().toLowerCase(),
    name: String(r[at('Full Name')] ?? '').trim(),
    unsubscribed: ticked(r[at('Unsubscribed')]),
    token: String(r[at('Token')] ?? '').trim(),
  })).filter((r) => r.email);

  return { rows, missingTab: false };
}

/** Everyone whose Unsubscribed box is not ticked. The only list that is ever invited. */
export const activeSubscribers = (rows) => rows.filter((r) => !r.unsubscribed);

/* -------------------------------------------------------------------- write */

/**
 * Writes the header row, pushing any existing rows down first so nothing is overwritten.
 * Inserting a row needs the tab's numeric id, which only the spreadsheet metadata carries.
 */
async function writeHeader(cfg, t, pushDown) {
  const auth = { authorization: `Bearer ${t}`, 'content-type': 'application/json' };

  if (pushDown) {
    const meta = await fetch(
      `${SHEETS_API}/${encodeURIComponent(cfg.spreadsheetId)}?fields=sheets.properties`,
      { headers: { authorization: `Bearer ${t}` } });
    if (!meta.ok) throw new SheetError('write_failed', 'Could not read the tab list.',
      { publicMessage: 'That did not work just now. Try again in a moment.' });
    const { sheets = [] } = await meta.json();
    const tab = sheets.find((x) => x.properties?.title === TAB);
    if (!tab) throw new SheetError('no_tab', `${TAB} disappeared.`,
      { publicMessage: 'Signing up is not switched on yet. Take the calendar feed instead.' });

    const insert = await fetch(
      `${SHEETS_API}/${encodeURIComponent(cfg.spreadsheetId)}:batchUpdate`,
      { method: 'POST', headers: auth,
        body: JSON.stringify({ requests: [{ insertDimension: {
          range: { sheetId: tab.properties.sheetId, dimension: 'ROWS',
                   startIndex: 0, endIndex: 1 } } }] }) });
    if (!insert.ok) throw new SheetError('write_failed', 'Could not make room for a header.',
      { publicMessage: 'That did not work just now. Try again in a moment.' });
  }

  const res = await fetch(
    `${SHEETS_API}/${encodeURIComponent(cfg.spreadsheetId)}/values/`
    + `${encodeURIComponent(`${TAB}!A1:E1`)}?valueInputOption=RAW`,
    { method: 'PUT', headers: auth, body: JSON.stringify({ values: [COLUMNS] }) });
  if (!res.ok) throw new SheetError('write_failed', `Could not write the header (${res.status}).`,
    { publicMessage: 'That did not work just now. Try again in a moment.' });
}

/**
 * Adds a subscriber, or reactivates one who had left.
 *
 * Returns the same shape whether the address was new or already there, and never says which.
 * "That address is already subscribed" turns the form into a way of testing whether somebody
 * is a member of this chapter.
 */
export async function subscribe(env, { email, name = '' }) {
  const cfg = config(env);
  if (!ready(cfg)) {
    return { ok: false, reason: 'not_enabled',
             message: 'Signing up is not switched on yet. Take the calendar feed instead.' };
  }
  const address = String(email ?? '').trim().toLowerCase();
  if (!looksLikeEmail(address)) {
    return { ok: false, reason: 'bad_email',
             message: 'That does not look like an email address — check it and try again.' };
  }

  let { rows, missingTab, needsHeader, firstRowIsData } = await readSubscribers(env);
  if (missingTab) {
    return { ok: false, reason: 'no_tab',
             message: 'Signing up is not switched on yet. Take the calendar feed instead.',
             log: `the ${TAB} tab does not exist — run EO Calendar → Set up subscriptions` };
  }

  const t = await token(cfg, SCOPES.sheetsWrite);

  // Put the header in place rather than refusing. A tab somebody made by hand is the normal
  // case, and it is the only thing standing between here and a working list.
  if (needsHeader) {
    await writeHeader(cfg, t, firstRowIsData);
    ({ rows } = await readSubscribers(env));
  }
  const auth = { authorization: `Bearer ${t}`, 'content-type': 'application/json' };
  const existing = rows.find((r) => r.email === address);
  const now = new Date().toISOString();

  if (existing) {
    if (!existing.unsubscribed) {
      return { ok: true, already: true, token: existing.token,
               message: 'You are on the list.' };
    }
    // Coming back: untick the box in place rather than leaving two rows for one person.
    const fresh = existing.token || newToken();
    const res = await fetch(
      `${SHEETS_API}/${encodeURIComponent(cfg.spreadsheetId)}/values/`
      + `${encodeURIComponent(`${TAB}!B${existing.rowNumber}:E${existing.rowNumber}`)}`
      + '?valueInputOption=RAW',
      { method: 'PUT', headers: auth,
        body: JSON.stringify({ values: [[now, name || existing.name, false, fresh]] }) });
    if (!res.ok) {
      throw new SheetError('write_failed', `Could not resubscribe (${res.status}).`,
        { publicMessage: 'That did not work just now. Try again in a moment.' });
    }
    return { ok: true, token: fresh, message: 'You are on the list.' };
  }

  const fresh = newToken();

  // Written to the first row with no address in it, NOT appended.
  //
  // `values.append` puts the row after the last cell holding anything at all — and the setup
  // routine turns column D into checkboxes down the whole sheet, so a thousand blank rows each
  // hold a FALSE and the first subscriber lands at row 1003. The sheet looks empty and the
  // list looks broken. The Email column is the only thing that says whether a row is used.
  const target = rows.length ? Math.max(...rows.map((r) => r.rowNumber)) + 1 : 2;
  const res = await fetch(
    `${SHEETS_API}/${encodeURIComponent(cfg.spreadsheetId)}/values/`
    + `${encodeURIComponent(`${TAB}!A${target}:E${target}`)}?valueInputOption=RAW`,
    { method: 'PUT', headers: auth,
      body: JSON.stringify({ values: [[address, now, name, false, fresh]] }) });

  if (!res.ok) {
    const detail = await res.text().catch(() => '');
    throw new SheetError('write_failed',
      `Could not add the subscriber (${res.status}). ${detail.slice(0, 200)}`,
      { publicMessage: 'That did not work just now. Try again in a moment.' });
  }
  return { ok: true, token: fresh, message: 'You are on the list.' };
}

/**
 * Marks a subscriber as gone, by token or by address.
 *
 * BY TOKEN is the good path: a random string that identifies one person, so nobody can remove
 * anybody else. A token can never travel in a calendar invitation, though — an event has ONE
 * description shared by every guest, so the link in there cannot be personal. Somebody
 * arriving from it has no token and would otherwise be stuck.
 *
 * BY ADDRESS is therefore accepted too, from the page's own form. It is weaker: knowing an
 * address is enough to remove it. That is the same standing as most "manage your preferences"
 * pages without a login, and the harm is bounded — the person can rejoin in one click, and
 * nothing is disclosed either way, because the answer is identical whether the address was on
 * the list or not.
 *
 * A token in a URL is never accepted as an address, or the weaker path would undermine the
 * stronger one.
 */
export async function unsubscribe(env, { token: tokenValue, email } = {}) {
  const cfg = config(env);
  const value = String(tokenValue ?? '').trim();
  const address = String(email ?? '').trim().toLowerCase();

  if (!ready(cfg) || (!value && !address)) {
    return { ok: false, reason: 'bad_token',
             message: 'That unsubscribe link is not valid. Type your address below instead.' };
  }
  if (!value && !looksLikeEmail(address)) {
    return { ok: false, reason: 'bad_email',
             message: 'That does not look like an email address — check it and try again.' };
  }

  const { rows } = await readSubscribers(env);
  const found = value
    ? rows.find((r) => r.token && r.token === value)
    : rows.find((r) => r.email === address);

  // The same answer either way. A wrong token must not reveal that a right one exists, and an
  // address that is not on the list must not be confirmed as absent.
  if (!found) {
    return value
      ? { ok: false, reason: 'bad_token',
          message: 'That unsubscribe link is not valid. Type your address below instead.' }
      : { ok: true, already: true, message: 'You will not be invited again.' };
  }
  if (found.unsubscribed) {
    return { ok: true, already: true, email: found.email,
             message: 'You were already unsubscribed — nothing more to do.' };
  }

  const t = await token(cfg, SCOPES.sheetsWrite);
  const res = await fetch(
    `${SHEETS_API}/${encodeURIComponent(cfg.spreadsheetId)}/values/`
    + `${encodeURIComponent(`${TAB}!D${found.rowNumber}`)}?valueInputOption=RAW`,
    { method: 'PUT',
      headers: { authorization: `Bearer ${t}`, 'content-type': 'application/json' },
      body: JSON.stringify({ values: [[true]] }) });

  if (!res.ok) {
    throw new SheetError('write_failed', `Could not unsubscribe (${res.status}).`,
      { publicMessage: 'That did not work just now. Try again in a moment.' });
  }
  return { ok: true, email: found.email, message: 'You will not be invited again.' };
}
